...
install_core() {
    ...
    yellow "准备权限..."
    cd /root/
    chmod +x ${XUI_DIR}/x-ui ${XUI_DIR}/bin/xray-linux-amd64 ${XUI_DIR}/x-ui.sh
    ...
    cp -f ${XUI_DIR}/x-ui.service /etc/systemd/system/
    mv ${XUI_DIR}/ /usr/local/
    ...
    /usr/local/x-ui/x-ui db init || yellow "db初始化跳过"
