...
ssl_cert_issue() {
    ...
    # 2. 安装必要依赖 (只有进入此函数才会执行)
    LOGI "正在准备签发环境，安装 socat..."
    case "${release}" in 
        ubuntu | debian | armbian) 
    ...
    # 6. 设置验证端口 (默认 80)
    local WebPort=80 
    read -rp "请选择用于验证的端口 (回车默认为 80): " WebPort 
    [[ -z "${WebPort}" ]] && WebPort=80
    LOGI "将使用端口 ${WebPort} 进行签发，请确保该端口已在防火墙放行。" 

    # 7. 签发证书 
    ~/.acme.sh/acme.sh --set-default-ca --server letsencrypt 
    ~/.acme.sh/acme.sh --issue -d ${domain} --listen-v6 --standalone --httpport ${WebPort} --force 
    if [ $? -ne 0 ]; then 
        LOGE "签发失败！原因：1. 80端口被占用；2. 域名解析未生效；3. 防火墙未关。" 
        return 1 
    fi 
    ...
    # 10. 自动绑定到面板
    echo -e "\n${green}证书签发成功！正在为您自动配置面板 HTTPS...${plain}"
    local webCertFile="/root/cert/${domain}/fullchain.pem" 
    local webKeyFile="/root/cert/${domain}/privkey.pem" 
    ...
    if [[ -f "$webCertFile" && -f "$webKeyFile" ]]; then 
        ...
        LOGI "已成功配置证书路径。" 
        echo -e "——————————————————————"
        echo -e "${green}登录地址: https://${domain}:${p_port}${p_path}${plain}" 
        echo -e "——————————————————————"
        restart 
    else 
        LOGE "绑定失败：未找到生成的证书文件。" 
    fi 
}
ssl_cert_issue_CF() { 
     local existing_webBasePath=$(/usr/local/x-ui/x-ui setting -show true | grep -Eo 'webBasePath（访问路径）: .+' | awk '{print $2}') 
