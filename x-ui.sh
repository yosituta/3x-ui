...
ssl_cert_issue() {
    ...
    case "${release}" in 
        ...
        arch | manjaro | parch) 
            ...
            ;; 
        *) 
            echo -e "${red}不支持的操作系统。请手动安装 socat。${plain}\n" 
            return 1 
            ;; 
    esac
    ...
    # 6. 设置验证端口 
    local WebPort=80 
    read -rp "请选择验证端口 (默认为 80): " WebPort 
    [[ -z "${WebPort}" ]] && WebPort=80
    LOGI "将使用端口: ${WebPort} 签发证书，请确保防火墙已开放此端口。" 

    # 7. 签发证书 
    ~/.acme.sh/acme.sh --set-default-ca --server letsencrypt 
    ~/.acme.sh/acme.sh --issue -d ${domain} --listen-v6 --standalone --httpport ${WebPort} --force 
    if [ $? -ne 0 ]; then 
        LOGE "签发证书失败，请确保域名已解析到此服务器。" 
        return 1 
    fi 
